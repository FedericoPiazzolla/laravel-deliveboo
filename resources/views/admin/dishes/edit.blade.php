...
<div class="container mt-5">
    ...
    <h2 class="text-center">Modifica i dettagli del piatto</h2>
    ...
    <form name="myForm" class="mt-5" action="{{ route('admin.dishes.update', ['dish' => $dish->slug]) }}" onsubmit="return validateForm()" method="POST" enctype="multipart/form-data" id="dishCreationForm">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control dish_name" id="name" name="name" value="{{ old('name') ?? $dish->name }}" required>
            <span>
        ...
        <div class="mb-3">
            ...
            <input type="file" class="form-control @error('image') is-invalid @enderror" id="image"
                name="image" value="{{ old('image') ?? $dish->image }}" onchange="showImage(event)" accept=".jpg, .jpeg, .svg, .png, .bpm, .gif, .webp">
        </div>
        ...
        <div class="input-group">
            ...
            <input name="price" id="price" type="text" class="form-control" aria-label="Dollar amount (with dot and two decimal places)" value="{{ old('price') ?? $dish->price }}" pattern="^\d{1,4}(\.\d{2})?$" onblur="this.value = parseFloat(this.value).toFixed(2)" required>
        </div>
        ...
        <div class="mt-3">
            <button class="btn btn-warning" type="submit" id="dishCreateBtn">Applica</button>
        </div>
    </form>
